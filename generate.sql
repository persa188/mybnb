...
-- -----------------------------------------------------
DROP TABLE IF EXISTS `mybnb`.`cancelled` ;
...
USE `mybnb`$$
DROP TRIGGER IF EXISTS `mybnb`.`accounts_BEFORE_DELETE` $$
USE `mybnb`$$
CREATE
DEFINER=`root`@`localhost`
TRIGGER `mybnb`.`accounts_BEFORE_DELETE`
BEFORE DELETE ON `mybnb`.`accounts`
FOR EACH ROW
BEGIN
	DELETE FROM mybnb.renter WHERE renter.user = OLD.user;
    DELETE FROM mybnb.host WHERE host.user = OLD.user;
    DELETE FROM mybnb.lives WHERE lives.user = OLD.user;
END$$


USE `mybnb`$$
DROP TRIGGER IF EXISTS `mybnb`.`cancelled_AFTER_INSERT` $$
USE `mybnb`$$
CREATE
DEFINER=`root`@`localhost`
...
USE `mybnb`$$
DROP TRIGGER IF EXISTS `mybnb`.`host_BEFORE_DELETE` $$
USE `mybnb`$$
CREATE
DEFINER=`root`@`localhost`
TRIGGER `mybnb`.`host_BEFORE_DELETE`
BEFORE DELETE ON `mybnb`.`host`
FOR EACH ROW
BEGIN
	DELETE FROM mybnb.hosts WHERE hosts.user = OLD.user;
END$$


USE `mybnb`$$
DROP TRIGGER IF EXISTS `mybnb`.`renter_BEFORE_DELETE` $$
USE `mybnb`$$
CREATE
DEFINER=`root`@`localhost`
TRIGGER `mybnb`.`renter_BEFORE_DELETE`
BEFORE DELETE ON `mybnb`.`renter`
FOR EACH ROW
BEGIN
	DELETE FROM mybnb.rents WHERE rents.ruser = OLD.user;
END$$
